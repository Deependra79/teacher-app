...
export async function getChatList(userId) {
  ...
  const callerRole = isStudent ? "student" : (isTeacher ? "teacher" : null);

  const list = [];
  for (const partnerId of uniquePartners) {
    // Check if the chat was deleted by the user and not recreated by subsequent messages
    const { data: lastMsgs, error: lastErr } = await supabase
      .from("messages")
      .select("*")
      .or(`and(sender_id.eq.${idNum},receiver_id.eq.${partnerId}),and(sender_id.eq.${partnerId},receiver_id.eq.${idNum})`)
    ...
    if (
      lastMsg &&
      Number(lastMsg.sender_id) === idNum &&
      lastMsg.message === "__SYSTEM__ACTION__DELETE__"
    ) {
      continue;
    }
    ...
    if (callerRole === "teacher") {
      // Caller is a teacher, partner must be a student
      ...
      // Caller is a student, partner must be a teacher
    ...
    const blockStatus = await isBlocked(idNum, partnerId);
    list.push({ 
      chat_user_id: partnerId, 
      name,
...
export async function isBlocked(user1, user2) {
  ...
  const { data, error } = await supabase
    ...
    .in("message", ["__SYSTEM__ACTION__BLOCK__", "__SYSTEM__ACTION__UNBLOCK__"])
    .order("id", { ascending: false })
    .limit(1);
  ...
  const latest = data?.[0];
  if (latest && latest.message === "__SYSTEM__ACTION__BLOCK__") {
    return { blocked: true, blockerId: Number(latest.sender_id) };
  }
